...
fn string_equality_program() -> CompiledProgram {
    CompiledProgram::new(
        PrimeId::Bn254,
        ...
        Function {
            ...
            constants: vec![Value::string(0), Value::string(1)],
            upvalue_info: Vec::new(),
            line_info: vec![1; 4],
        },
        HashMap::new(),
...
#[test]
fn orc_jit_uses_runtime_equality_for_distinct_heap_handles() {
    let program = string_equality_program();
    let engine = JitEngine::compile(&program).unwrap();
    let mut vm = VM::new();
    vm.load_program(program).unwrap();
    ...
    assert_eq!(result.outcome, ExecutionOutcome::Native);
    assert_eq!(result.stats.runtime_calls, 3);
    assert_eq!(result.stats.interpreter_fallbacks, 0);
}
