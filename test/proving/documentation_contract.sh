...
rg -Fq 'commit-beacon.mjs' "$trusted"
rg -Fq 'fetch-beacon.mjs' "$trusted"
rg -Fq -- '--beacon-evidence' "$trusted"
rg -Uiq 'publish.*commitment.*before.*target' "$trusted"
rg -Uiq 'public beacon\s+does not replace\s+an\s+independent' "$trusted"
rg -Fq -- '--contributed-zkey' "$boss"
rg -Fq -- '--beacon-evidence' "$boss"
